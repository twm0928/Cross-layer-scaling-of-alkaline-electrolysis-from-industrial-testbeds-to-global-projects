...
%DISPATCH_MILP_STATIC_MAP Plant dispatch using an integer P-H2 map.
%
% For each time step, solve the following bounded-integer linear model:
%   maximise   sum_k n_k h_k
%   subject to sum_k n_k p_k <= P_available
%              sum_k n_k <= N_modules
%              n_k are non-negative integers
%
% The implementation uses dynamic programming over finely discretised module
% loading levels for speed. It is equivalent to the corresponding bounded
% integer linear model because there are no inter-temporal constraints in
% this benchmark.
